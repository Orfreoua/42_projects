...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/08/19 17:25:08 by orfreoua          #+#    #+#             */
/*   Updated: 2023/08/19 17:25:09 by orfreoua         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double	*ft_se_x(t_data *data, double x, double y, double angle)
{
			...
			data->player.hit1_x, data->player.hit1_y) != '1')
	{
		...
		data->player.hit1_x = (int)data->player.x + i + 1;
		data->player.hit1_y = (int)data->player. y + y + sqrt(
				hypo[0] * hypo[0] - (((1 - x) + i) * ((1 - x) + i)));
		i++;
	}
	hypo[1] = ft_se_x_1(data, x, y, angle);
	return (hypo);
}
...
double	ft_se_x_1(t_data *data, double x, double y, double angle)
{
			...
			data->player.hit2_x, data->player.hit2_y) != '1')
	{
		...
		data->player.hit2_y = (int)data->player.y + i + 1;
		i++;
	}
	return (hypo);
}
